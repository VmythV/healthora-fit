...
export const aiConfigQueries = {
   ...
   */
  async save(config: { apiEndpoint: string; apiKey: string; modelName: string }): Promise<number> {
    const db = database.getDatabase()
  ...
  async update(
    ...
    if (updates.apiKey !== undefined) {
      ...
      // 清理旧句柄
      const all = await db.getAllAsync<any>(`SELECT api_key FROM ai_config WHERE id = ?`, [id])
  ...
  async testConnection(): Promise<{ success: boolean; error?: string }> {
    ...
    const apiType: 'chat-completions' | 'responses' | undefined = config.apiEndpoint?.endsWith(
      '/responses'
    )
      ? 'responses'
      : undefined
