...
function getWeekRange(weekOffset: number = 0): DateRange {
  ...
  return {
    ...
    label: weekOffset === 0 ? '本周' : weekOffset === -1 ? '上周' : `${formatLabel(monday)} - ${formatLabel(sunday)}`,
  };
}
...
export function TimeRangeSelector({
  ...
  const handleTypeChange = (type: TimeRangeType) => {
    setRangeType(type);
    setOffset(0);
    const range = type === 'week' ? getWeekRange(0) : type === 'month' ? getMonthRange(0) : getYearRange(0);
    onRangeChange(range);
  };
  ...
  const handlePrev = () => {
    ...
    setOffset(newOffset);
    const range = rangeType === 'week' ? getWeekRange(newOffset) : rangeType === 'month' ? getMonthRange(newOffset) : getYearRange(newOffset);
    onRangeChange(range);
  };
  ...
  const handleReset = () => {
    setOffset(0);
    const range = rangeType === 'week' ? getWeekRange(0) : rangeType === 'month' ? getMonthRange(0) : getYearRange(0);
    onRangeChange(range);
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.typeSelector}>
        {(['week', 'month', 'year'] as TimeRangeType[]).map((type) => (
          <TouchableOpacity
            ...
            style={[
              styles.typeButton,
              rangeType === type && styles.typeButtonActive,
            ]}
          ...
          >
            <Text
              style={[
                ...
                rangeType === type && styles.typeButtonTextActive,
              ]}
            >
              {t(`analysis.${type}`)}
            </Text>
      ...
      <View style={styles.rangeNavigator}>
        ...
        <TouchableOpacity onPress={handleReset} style={styles.rangeLabel}>
          ...
          {offset !== 0 && (
            <Text style={styles.resetHint}>点击回到当前</Text>
          )}
        </TouchableOpacity>
