...
describe('P3-38: goals.set 两段法', () => {
  it('goalQueries.set 改用 SELECT keepIds 后 DELETE（不再用子查询 NOT IN）', () => {
    const src = fs.readFileSync(
      path.resolve(__dirname, '../database/queries/goals.ts'),
      'utf8'
    );
...
describe('P3-39: getRecent 抽 MAX_EXPORT_RECORDS', () => {
  it('diet.ts 与 weight.ts 导出 MAX_EXPORT_RECORDS', () => {
    const dietSrc = fs.readFileSync(
      path.resolve(__dirname, '../database/queries/diet.ts'),
      'utf8'
    );
  ...
  it('dataTransfer.ts 用常量替代硬编码 10000', () => {
    const src = fs.readFileSync(
      path.resolve(__dirname, '../services/dataTransfer.ts'),
      'utf8'
    );
    // 不应有裸 "10000" 数字（除注释外）
...
describe('P3-42: headerTitle i18n', () => {
  ...
  it('3 个 modal 屏 useLayoutEffect setOptions', () => {
    ...
    expect(diet).toMatch(/router\.setOptions\(\{\s*title:\s*t\(['"]diet\.recordTitle['"]/);
    expect(ex).toMatch(/router\.setOptions\(\{\s*title:\s*t\(['"]exercise\.recordTitle['"]/);
    expect(wt).toMatch(/router\.setOptions\(\{\s*title:\s*t\(['"]weight\.recordTitle['"]/);
  });
...
describe('P3-44: expo-image 替换', () => {
  it('DietRecordDetail.tsx 与 ScreenshotPicker.tsx 用 expo-image', () => {
    const diet = fs.readFileSync(path.resolve(__dirname, '../components/diet/DietRecordDetail.tsx'), 'utf8');
    const ex = fs.readFileSync(path.resolve(__dirname, '../components/exercise/ScreenshotPicker.tsx'), 'utf8');
...
// ===== P3-46: 删 useNotification 桥接 =====
describe('P3-46: useNotification 桥接删除', () => {
  it('useNotification.tsx 不再调 setGlobalNotify/setGlobalConfirm', () => {
    const src = fs.readFileSync(path.resolve(__dirname, '../hooks/useNotification.tsx'), 'utf8');
    expect(src).not.toContain('setGlobalNotify');
    expect(src).not.toContain('setGlobalConfirm');
  });
});
