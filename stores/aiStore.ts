...
interface AIState {
  ...
  saveConfig: (config: { apiEndpoint: string; apiKey: string; modelName: string }) => Promise<void>
...
export const useAIStore = create<AIState>((set, _get) => ({
  ...
  // 设置分析状态
  setAnalyzing: (isAnalyzing, progress = 0) => set({ isAnalyzing, analysisProgress: progress }),
